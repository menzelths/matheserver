$(document).ready(function () {
    ...
    $("#ende").click(function () {
        eb.send("matheserver", {typ: "einaus"});
    });
    ...
    eb.onopen = function () {
        ...
        eb.registerHandler('matheserver.spielfeld', function (message, replier) {
            ...
            if (typ === "id" && anmeldung === true) {
                ...
                if (an > -1) {
                    eb.send("matheserver.spieler." + spielerListe2[nr].uuid, {typ: "neueAufgabe", wert: aufgaben[an], richtig: spielerListe2[nr].richtig, gesamt: spielerListe2[nr].gesamt, platz: -1, anzahlSpieler: spielerListe2.length, anzahlAufgaben: anzahlAufgaben});
                } else {
                    fertig = true;
                }
    ...
    $("#knopf").click(function () {
        ...
        for (var i = 0; i < spielerListe2.length; i++) {
            ...
            eb.send("matheserver.spieler." + spielerListe2[i].uuid, {typ: "neueAufgabe", wert: aufgaben[nr], richtig: 0, gesamt: 0, platz: -1, anzahlSpieler: spielerListe2.length, anzahlAufgaben: anzahlAufgaben});
        }
    });
